...
If you want this MVP in a dedicated folder (for example `~/supportops-mvp`), copy it out of this repo first:

```bash
mkdir -p ~/supportops-mvp
cp -R /path/to/ai-agents-for-beginners/supportops-mvp/* ~/supportops-mvp/
```

Then continue the setup steps from inside `~/supportops-mvp`.

## Local Setup (Ubuntu 22.04 + MySQL)

### 1) Backend (Laravel)

Follow the instructions in [`supportops-mvp/backend/README.md`](./backend/README.md) to scaffold a Laravel app and copy in the MVP files.

### 2) Frontend (Astro)

```bash
cd supportops-mvp/frontend
npm install
npm run dev
```
